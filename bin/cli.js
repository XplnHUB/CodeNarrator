...
async function main() {
  program
    ...
    .version(version)
    .description(
      "A tool that automatically generates documentation for JavaScript codebases using AI"
    )
    .argument(
      "[path]",
      "path to code/project folder (or configure in .codenarratorrc.json)"
    )
    .option("-o, --output <folder>", "output folder for documentation")
    .option(
      "--model <name>",
      "AI model to use (currently only gemini is supported)"
    )
